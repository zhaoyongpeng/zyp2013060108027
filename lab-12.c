...
int main(int argc, char *argv[]) {
	...
	if((emptysem = sem_open("emptysem", O_CREAT, S_IRWXO, 10)) == SEM_FAILED) {
        perror("failed");
	}
    if((fullsem = sem_open("fullsem", O_CREAT, S_IRWXO, 0)) == SEM_FAILED) {
        perror("failed");
    }

    while (1) {

        if (sem_trywait(emptysem) == 0) {
            ...
            state = waiting;
            printf("producer %d: waiting, can't go into critical section\n", id);
        }
        sleep(1);
    }
